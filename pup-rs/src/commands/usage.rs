...
use datadog_api_client::datadogV1::api_usage_metering::{
    UsageMeteringAPI, GetUsageSummaryOptionalParams, GetHourlyUsageAttributionOptionalParams,
};
...
pub async fn summary(cfg: &Config, start: String, end: Option<String>) -> Result<()> {
    ...
    let start_dt = chrono::DateTime::from_timestamp_millis(
        util::parse_time_to_unix_millis(&start)?,
    )
    .unwrap();
...
pub async fn hourly(cfg: &Config, start: String, end: Option<String>) -> Result<()> {
    ...
    let resp = api
        .get_hourly_usage_attribution(
            start_dt,
            HourlyUsageAttributionUsageType::API_USAGE,
            params,
        )
        .await
